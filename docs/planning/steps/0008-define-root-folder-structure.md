0008-define-root-folder-structure.md

## Step 8 — Record the canonical repository structure decision

Now create **ADR-0003: Use a governance-grade root folder structure**.

You already created the folders. This step records *why* the repo uses a broad structure from the beginning instead of starting with only `apps/` and `packages/`. The charter explicitly defines separate homes for apps, services, packages, libs, tools, configs, contracts, infra, policies, docs, tests, examples, templates, profiles, scripts, GitHub config, Dev Container config, and `.moro` metadata. 

From the repo root, run:

````bash
cat > docs/architecture/decisions/adr-0003-use-governance-grade-root-folder-structure.md <<'EOF'
...
```bash
git add docs/architecture/decisions/adr-0003-use-governance-grade-root-folder-structure.md
git commit -m "docs(adr): define root folder structure"
git push
````
