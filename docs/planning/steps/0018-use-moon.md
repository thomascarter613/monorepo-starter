...
## Step 18 — Record the monorepo task-runner decision

Now create **ADR-0006: Use moon for monorepo task orchestration**.

Before installing or configuring moon, record the decision. Your charter names **moon** as the default monorepo task runner, with Turborepo, Nx, and Lage as alternatives. Because task orchestration will eventually control `check`, `build`, `test`, `typecheck`, CI workflows, dependency graphs, and project boundaries, this is a foundational decision. 

From the repo root, run:

````bash
cat > docs/architecture/decisions/adr-0006-use-moon-for-monorepo-task-orchestration.md <<'EOF'
...
```bash
git add docs/architecture/decisions/adr-0006-use-moon-for-monorepo-task-orchestration.md
git commit -m "docs(adr): choose moon for monorepo task orchestration"
git push
````
