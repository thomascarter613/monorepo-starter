0014-recoord-primary-language.md

## Step 14 — Record the primary language decision

Now create **ADR-0004: Use TypeScript as the primary language layer**.

Before adding `tsconfig`, Biome, moon tasks, app scaffolds, service code, or the `moro` CLI, record that TypeScript is the project’s default implementation language. Your charter defines the starter as Bun-first and TypeScript-first, with the `moro` CLI written in TypeScript and run on Bun. 

From the repo root, run:

````bash
cat > docs/architecture/decisions/adr-0004-use-typescript-as-primary-language-layer.md <<'EOF'
...
```bash
git add docs/architecture/decisions/adr-0004-use-typescript-as-primary-language-layer.md
git commit -m "docs(adr): choose typescript as primary language layer"
git push
````
